...
public:
	...
	void update();
	void draw(ofColor color = ofColor::gray);
...
protected:
	...
	int* m_pOpenVoxels = NULL;

	int m_nGridSize;
	int m_nGridSizep1;
	float m_fVoxelSize;

	float* m_pfGridEnergy = NULL;
	char* m_pnGridPointStatus = NULL;
	char* m_pnGridVoxelStatus = NULL;
	...
	int m_nNumVertices;
	int m_nNumIndices;
	SVertex* m_pVertices = NULL;
	unsigned short* m_pIndices = NULL;
};
